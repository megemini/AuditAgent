...
class ReimbursementRulesManager:
    """财务报销规则管理器"""
...
报销制度|适用范围|1.2|适用费用|与公司经营活动直接相关的差旅、业务招待、采购、培训、通讯、加班餐费等可报销。|[费用类别表]
...
报销制度|报销原则|2.1|真实性|所有报销须基于真实业务，禁止虚构或拆分发票。|[合规声明]
报销制度|报销原则|2.3|时效性|费用发生后 30 天内提交系统，逾期视为自动放弃。|[系统截图]
差旅费|报销时限|T2.1|返京后报销|差旅费应于返京后五个工作日内办理报销手续，无特殊情况，前账不清，不予再借。|[报销时限提醒]
...
报销制度|票据规范|4.1|发票要求|须为国家税务局监制的增值税发票，抬头、税号、项目、金额完整。|[发票样例]
报销制度|票据规范|4.3|遗失处理|发票遗失须填写《票据遗失说明》，附付款凭证及对方单位盖章确认件。|[模板]
报销制度|付款时限|7.1|财务审核|财务部 5 个工作日内完成票据合规性及预算余额审核。|[SLA 协议]
报销制度|付款时限|7.2|打款|审核通过后 8 个工作日内网银付款至员工工资卡。|[银行回单]
报销制度|违规处理|8.2|重复报销|系统已自动校验，人工复核发现重复即冻结账号并通报。|[系统规则]
报销制度|附则|9.1|制度更新|财务部每年 12 月组织复审，经董事会批准后于次年 1 月生效。|[版本记录]
"""
        
        self.parse_rules_from_text(default_rules_text)
        logger.info("默认财务报销规则加载完成")
    ...
    def get_rules_by_subcategory(self, category: str, subcategory: str) -> List[ReimbursementRule]:
        """根据子分类获取规则"""
    ...
    def import_rules(self, file_path: str, replace: bool = False) -> bool:
        ...
        try:
            ...
            for rule_data in rules_data:
                try:
                    rule = ReimbursementRule(**rule_data)
...
# 全局规则管理器实例
rules_manager = ReimbursementRulesManager()

def get_rules_manager() -> ReimbursementRulesManager:
    """获取全局规则管理器实例"""
...
def search_rules(keyword: str) -> List[ReimbursementRule]:
    """搜索规则的便捷函数"""
    return rules_manager.search_rules(keyword)

def get_rules_by_category(category: str) -> List[ReimbursementRule]:
    """根据分类获取规则的便捷函数"""
